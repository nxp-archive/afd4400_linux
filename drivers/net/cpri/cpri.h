...
#define DEV_NAME		"cp"
#define FRMR_NAME		"frmr"
#define MAX_FRAMERS_PER_COMPLEX		2
#define MAX_CPRI_COMPLEXES		2

/* Debug and error reporting macros */
#define IF_DEBUG(x)     if (framer->debug & (x))
#define DEBUG(flg, txt, ...) {if (framer->debug & flg) \
			pr_info("%s: " txt, framer->name, ##__VA_ARGS__); }
#define ERR(txt, ...)        {if (framer->debug & DEBUG_MESSAGES) \
			pr_err("%s: " txt, framer->name, ##__VA_ARGS__); }

/* Debug bit assignments */
#define DEBUG_MESSAGES			(1<<0)
#define DEBUG_AUTONEG			(1<<1)
#define DEBUG_SFP			(1<<2)
#define DEBUG_IOCTL			(1<<3)
...
struct cpri_dev {
	...
	struct cpri_framer *framer[MAX_FRAMERS_PER_COMPLEX];
};
