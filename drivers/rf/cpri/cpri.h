/*
 * drivers/rf/cpri/cpri.h
 * CPRI device driver
 * Author: Freescale semiconductor, Inc.
 *
 * Copyright 2013 Freescale Semiconductor, Inc.
 *
 * This program is free software; you can redistribute  it and/or modify it
 * under  the terms of  the GNU General  Public License as published by the
 * Free Software Foundation;  either version 2 of the  License, or (at your
 * option) any later version.
 */

#ifndef __CPRIDRV_H
#define __CPRIDRV_H

#include <linux/cdev.h>
#include <linux/io.h>
#include <linux/platform_device.h>
#include <linux/etherdevice.h>
#include <linux/bitops.h>

#include "sfp.h"
#include "cpri_eth.h"

/* Autoneg data structures - Start */
enum cpri_state {
	/* Initial state */
	SFP_DETACHED = 1,
	STANDBY,
	/* State during autoneg*/
	LINE_RATE_AUTONEG,
	SERDES_ERROR,
	LINK_RATE_SETUP_TIMER_EXPIRED,
	PROT_VER_AUTONEG,
	PROT_VER_SETUP_TIMER_EXPIRED,
	ETHLINK_AUTONEG,
	ETHLINK_SETUP_TIMER_EXPIRED,
	PASSIVELINK,
	VENDOR_CONFIG,
	AUTONEG_COMPLETED,
	OPERATIONAL,
	/* Reconfiguration */
	LINK_RECONFIG_READY,
	INTERFACE_RECONFIG_READY,
	INTERFACE_RECONFIG_DONE,
	AXC_MAPPING_RECONFIG_READY,
	AXC_MAPPING_RECONFIG_DONE,
	/* Error state; can occur during operation */
	L1TIMER_EXPIRED,
	L1INBAND_ERROR,
	REC_RESET,
	ETHLINK_RATE_MISMATCH,
	ETHLINK_DISCONNECT,
	ETH_LINK_PROPOSED,
	PROT_VER_MISMATCH,
	INVALID_PARAM
};

enum cpri_link_rate {
	RATE2_1228_8M = 1,
	RATE3_2457_6M,
	RATE4_3072_0M,
	RATE5_4915_2M,
	RATE6_6144_0M,
	RATE7_9830_4M
};

enum cpri_cm_mode {
	ETH = 1,
	HDLC
};

enum cpri_prot_ver {
	VER_1 = 1,
	VER_2
};

enum hdlc_link_rate {
	RATE_0 = 1,
	RATE_240K,
	RATE_480K,
	RATE_960K,
	RATE_1920K,
	RATE_2400K,
	RATE_3840K,
	RATE_4800K,
	RATE_7680K
};

enum autoneg_cmd {
	CPRI_DO_LINE_RATE_AUTONEG = 1,
	CPRI_DO_PROTOCOL_AUTONEG,
	CPRI_DO_CNM_AUTONEG,
	CPRI_DO_VENDOR_AUTONEG,
	CPRI_DO_AUTONEG_ALL,
	CPRI_DO_RECONFIG
#define CPRI_LINK_RECONFIG_INIT_REQ		0x1
#define CPRI_INTERFACE_RECONFIG_INIT_REQ	0x2
#define CPRI_INTERFACE_RECONFIG_SETUP_REQ	0x4
#define CPRI_AXC_MAPPING_RECONFIG_INIT_REQ	0x8
#define CPRI_AXC_MAPPING_RECONFIG_SETUP_REQ	0x10
};

struct cpri_delay_config {
	u32 rx_ex_delay_period;
	u32 tx_ex_delay;
};

struct cpri_autoneg_param {
	u32 autoneg_param_flags;
	/* Line rate auto neg parameters */
	unsigned long linerate_autoneg_dur_sec;
	unsigned long tx_on_dur_sec;
	unsigned long tx_off_dur_sec;
	enum cpri_link_rate link_rate_low, link_rate_high;
	/*C&M rate auto neg parameters */
	unsigned long eth_setup_dur_sec;
	enum cpri_cm_mode cm_mode;
	enum hdlc_link_rate hdlc_rate_low, hdlc_rate_high;
	u8 eth_rate_cnt;
	u8 *eth_rate;
	/* Protocol auto neg parameters */
	unsigned long proto_ver_setup_dur_sec;
	enum cpri_prot_ver tx_prot_ver;
	u32 tx_scrseed_val;
};

struct cpri_delay_output {
	/* Extended delay measurement status */
	unsigned short rx_ex_buf_delay;
	unsigned char rx_ex_buf_delay_valid;
	/* Receive delay */
	unsigned char rx_byte_delay;
	unsigned char rx_buf_delay;
	unsigned char rx_align_delay;
	/* Round trip delay */
	u32 rx_roundtrip_delay;
};

struct cpri_autoneg_output {
	enum cpri_link_rate common_linkrate;
	u8 common_ethrate;
	enum cpri_prot_ver common_protver;
	unsigned char rx_eth_ptr;
	/* Basic frame */
	unsigned int cpri_bf_word_size;
	unsigned int cpri_bf_iq_datablock_size;
	/* Scramble seed */
	u32 rx_scrseed_val;
};
/* Autoneg data structures - End */

/* AxC data structures - Start */
enum mapping_method {
	METHOD_1 = 1,
	METHOD_2,
	METHOD_3
};

struct axc_rx_param {
	unsigned int axc_rx_param_flag;
	unsigned char sampling_width;
	unsigned int buffer_threshold;
	unsigned int S;
	unsigned char K;
	unsigned int Nst;
};

struct axc_tx_param {
	unsigned int axc_tx_param_flag;
	unsigned char sampling_width;
	unsigned int buffer_threshold;
	unsigned int S;
	unsigned char K;
	unsigned int Nst;
};

struct axc_buf {
	u32 addr;
	u32 size;
	unsigned int mem_blk;
	struct axc *axc;
	struct list_head list;
};

struct axc {
	unsigned int id;
	u32 flags;
	struct list_head list;
	struct cpri_framer *framer;
	enum mapping_method map_method;
	u8 axc_addr_word;
	u8 axc_addr_byte;
	struct axc_rx_param rx_param;
	struct axc_tx_param tx_param;
	struct axc_buf *axc_buf;
	unsigned int buffer_threshold;
};

struct axc_map_table {
	struct cpri_framer *framer;
	unsigned int max_time_slots;
	unsigned int slot_map_size;
	struct time_slot  *time_slots;
	u32 *time_slot_bitmap;
	unsigned int k0;
	unsigned int k1;
	u32 k0_bitmap[2];
	u32 k1_bitmap[2];
};

struct segment {
	struct axc *axc;
	u8 offset;
	u8 map_size;
};

struct time_slot {
	struct segment segments[3];
	unsigned int k;
	u8 segment_bitmap;
};

struct axc_mem_blk {
	u32 base;
	u32 size;
	u32 next_free_addr;
};

struct axc_buf_head {
	struct axc_mem_blk blocks[2];
	u8 blk_bitmap;
	u32 max_bufs;
	u32 allocated_bufs;
	struct list_head free_list;
	spinlock_t lock;
};
/* AxC data structures - End */

/* VSS data structures - Start */
struct vss_buffer_params {
	u32 flags;
	unsigned long buf_base_phys;
	unsigned int buff_count_hf;
	unsigned int buff_threshold_hf;
};
struct vss_buf_ctrl {
	u32  base_addr_phys;
	unsigned int buff_size;
	unsigned int curr_idx;
	unsigned int threshold;
};
/* VSS data structures - End */

/* Control word data structures - Start */
struct ctrl_chan {
	unsigned int chan;
	u8 words[4][16];
	u8 word_bitmap;
};
struct hf_ctrl_chans {
	struct ctrl_chan  **channels;
	unsigned int channel_count;
	unsigned int hf;
};
/* Control word data structures - End */

/* Daisy chain data structure */
struct daisy_chain_param {
	unsigned int flag;
	u8 hw_reset_bitmap;
	u8 cw130_bitmap;
	u32 axc_bitmap;
	u8 cw_bitmap[8];
};

/* Per CPRI complex data structures - Start */
struct event_listener {
	u32 evt_mask;
	unsigned int signal;
};

struct cpri_event_handler_list {
	u32 evt_mask;
	struct pid *pid;
	struct siginfo siginfo;
	struct list_head list;
};

struct cpri_dev {
	unsigned long dev_id;
	char *dev_name;
	struct cpri_common_regs __iomem *regs;
	struct device *dev;
	raw_spinlock_t lock;
	unsigned int irq_gen1;
	unsigned int irq_gen2;
	unsigned int irq_gen3;
	unsigned int irq_gen4;
	unsigned int irq_err;
	struct tasklet_struct *err_tasklet;
	unsigned int framers;
	struct list_head list;
	struct cpri_framer *framer[];
};

struct sfp_dev {
	u32 id;
	struct device_node *dev_node;
	unsigned int num_addresses;
	enum mem_type type;
	int use_smbus;
	struct sfp_info *info;
	struct cpri_framer *pair_framer;
	struct mutex lock;
	u8 *writebuf;
	unsigned write_max;
	/* TODO: stats */
	unsigned int irq_txfault;
	unsigned int irq_rxlos;
	unsigned int irq_prs;
	unsigned int tx_disable;
	struct list_head list;
	struct i2c_client *client[];
};

struct cpri_common_regs {
	u32 cpri_ctrlclk;	/* 0x7E00 - CPRI Control Clocks */
	u32 reserved0[(0x7e10 - 0x7e00) / sizeof(u32)];
	u32 cpri_intctrl[4];	/* 0x7E10 to 0x7E20 - CPRI Interrupt Control */
	u32 reserved1[(0x7E50 - 0x7E20) / sizeof(u32)];
	u32 cpri_rcpuctrlinten;	/* 0x7E50 - CPRI Rx CPU Ctrl Interrupt En */
	u32 cpri_tcpuctrlinten;	/* 0x7E54 - CPRI Tx CPU Ctrl Interrupt En */
	u32 cpri_rgensync;	/* 0x7E58 - Gen Rx Sync */
	u32 cpri_tgensync;	/* 0x7E5C - Gen Transmit Sync */
	u32 cpri_errstatus;	/* 0x7E94 - CPRI Err Status */
	u32 reserved2[(0x7eb8 - 0x7e94) / sizeof(u32)];
	u32 cpri_remresetoutputctrl; /* 0x7EB8 - CPRI Rem Reset Output Ctrl */
};
/* Per CPRI complex data structures - End */

/* Per CPRI framer data structures - Start */
struct cpri_framer_regs {
	/* Framer registers */
	u32 cpri_status;	/* 0x4  - Status */
	u32 cpri_config;	/* 0x8  - Configuration */
	u32 reserved0[(0x1c - 0x8) / sizeof(u32)];
	u32 cpri_lcv;		/* 0x1C - Rx Line Code Violation Count */
	u32 cpri_bfn;		/* 0x20 - Recovered BFN Counter */
	u32 cpri_hfn;		/* 0x24 - Recovered HFN Counter */
	u32 reserved1[(0x2c - 0x24) / sizeof(u32)];
	u32 cpri_hwreset;	/* 0x2C - HW Reset from Control Word */
	u32 reserved2[(0x3c - 0x2c) / sizeof(u32)];
	u32 cpri_cmconfig;	/* 0x3C - Control & Management Config */
	u32 cpri_cmstatus;	/* 0x40 - Control & Management Status */
	u32 reserved3[(0x48 - 0x40) / sizeof(u32)];
	u32 cpri_rdelay;	/* 0x48 - Rx Delay */
	u32 cpri_rnddelay;	/* 0x4C - Round Trip Delay */
	u32 cpri_exdelaycfg;	/* 0x50 - Extended Delay Measur Conf */
	u32 cpri_exdelaystatus;	/* 0x54 - Ext'd Delay Measurt Status */
	u32 cpri_tprotver;	/* 0x58 - Tx Protocol Version */
	u32 cpri_tscrseed;	/* 0x5C - Tx Scrambler Seed */
	u32 cpri_rscrseed;	/* 0x60 - Rx Scrambler Seed */
	u32 reserved4[(0x6c - 0x60) / sizeof(u32)];
	u32 cpri_tx_control;	/* 0x6C - Tx Control */
	u32 reserved5[(0x80 - 0x6c) / sizeof(u32)];
	u32 cpri_serdescfg;	/* 0x80 - SerDes Interface Config */
	u32 reserved6[(0xc0 - 0x80) / sizeof(u32)];
	u32 cpri_mapcfg;	/* 0xC0 - Mapping Configuration */
	u32 reserved7[(0xd0 - 0xc0) / sizeof(u32)];
	u32 cpri_maptblcfg;	/* 0xD0 - Mapping Table Config */
	u32 reserved8[(0xe4 - 0xd0) / sizeof(u32)];
	u32 cpri_rmapoffset;	/* 0xE4 - RX AxC Container Map Block Off */
	u32 cpri_tmapoffset;	/* 0xE8 - TX AxC Container Map Block Off */
	u32 reserved9[(0xf0 - 0xe8) / sizeof(u32)];
	u32 cpri_tstartoffset;	/* 0xF0 - Off for TX Start Sync Output */
	u32 reserved10[(0x100 - 0xF0) / sizeof(u32)];
	u32 cpri_riqbufstatus[2]; /* 0x100 to 0x104 - Map Buf RX Status */
	u32 reserved11[(0x120 - 0x104) / sizeof(u32)];
	u32 cpri_tiqbufstatus[2]; /* 0x120 tp 0x124 - Map Buf TX Status */
	u32 reserved12[(0x1c0 - 0x124) / sizeof(u32)];
	u32 cpri_rmapsmplcfg;	/* 0x1C0 - CPRI Rx AxC Sample Width Config */
	u32 cpri_tmapsmplcfg;	/* 0x1C4 - CPRI Tx AxC Sample Width Config */
	u32 reserved13[(0x1d0 - 0x1c4) / sizeof(u32)];
	u32 cpri_r1mapkselect;	/* 0x1D0 - CPRI Rx K Selection Reg1 */
	u32 cpri_r2mapkselect;	/* 0x1D4 - CPRI Rx K Selection Reg2 */
	u32 reserved14[(0x1e0 - 0x1d4) / sizeof(u32)];
	u32 cpri_t1mapkselect;	/* 0x1E0 - CPRI Tx K Selection Reg1 */
	u32 cpri_t2mapkselect;	/* 0x1E4 - CPRI Tx K Selection Reg2 */
	u32 reserved15[(0x200 - 0x1e4) / sizeof(u32)];
	u32 cpri_rethstatus;	/* 0x200 - Eth Receive Status */
	u32 reserved16[(0x208 - 0x200) / sizeof(u32)];
	u32 cpri_ethcfg1;	/* 0x208 - Eth Feat En/Dis and Trig En Bits */
	u32 cpri_ethcfg2;	/* 0x20C - Ethernet Miscellaneous Config */
	u32 cpri_rethctrl;	/* 0x210 - Ethernet RX Packet Discard */
	u32 reserved17[(0x228 - 0x210) / sizeof(u32)];
	u32 cpri_rethexstatus;	/* 0x228 - Ethernet RX External Status */
	u32 cpri_ethaddrmsb;	/* 0x22C - Ethernet 16 MSB of MAC Address */
	u32 cpri_ethaddrlsb;	/* 0x230 - Ethernet 32 LSB of MAC Address */
	u32 cpri_ethhashtbl;	/* 0x234 - Eth Hash Tbl to Filt Mult Traf */
	u32 reserved18[(0x244 - 0x234) / sizeof(u32)];
	u32 cpri_ethcfg3;	/* 0x244 - Ethernet Configuration 3 */
	u32 cpri_rethframercnt;	/* 0x248 - Ethernet Receive Frame Counter */
	u32 cpri_tethcntframer;	/* 0x24C - Ethernet Transmit Frame Counter */
	u32 cpri_rethdmamismcnt;/* 0x250 - Eth RX DMAC Mismatch Frame Counter*/
	u32 reserved19[(0x300 - 0x250) / sizeof(u32)];
	u32 cpri_rhdlcstatus;	/* 0x300 - HDLC Receive Status */
	u32 reserved20[(0x308 - 0x300) / sizeof(u32)];
	u32 cpri_hdlccfg1;	/* 0x308 - HDLC Diff Feat En/Dis & Trig En */
	u32 cpri_hdlccfg2;	/* 0x30C - HDLC Miscellaneous Configuration */
	u32 cpri_rhdlcctrl;	/* 0x310 - HDLC RX Packet Discard */
	u32 reserved21[(0x328 - 0x310) / sizeof(u32)];
	u32 cpri_rhdlcexstatus;	/* 0x328 - HDLC RX External Status */
	u32 reserved22[(0x344 - 0x328) / sizeof(u32)];
	u32 cpri_hdlccfg3;	/* 0x344 - HDLC Configuration 3 */
	u32 cpri_rhdlcframecnt;	/* 0x348 - HDLC Receive Frame Counter */
	u32 cpri_thdlcframecnt;	/* 0x34C - HDLC Transmit Frame Counter */
	/* DMA registers */
	u32 reserved23[(0x40c - 0x34c) / sizeof(u32)];
	u32 cpri_rvssaxisize;	/* 0x40C - Rx VSS AXI Transaction Size */
	u32 cpri_tvssaxisize;	/* 0x410 - Tx VSS AXI Transaction Size */
	u32 reserved24[(0x46c - 0x410) / sizeof(u32)];
	u32 cpri_rvssbufsize;	/* 0x46C - Rx VSS Buf Size */
	u32 cpri_tvssbufsize;	/* 0x470 - Tx VSS Buf Size */
	u32 cpri_rethbufsize;	/* 0x474 - Rx Eth Buf Size */
	u32 cpri_rhdlcbufsize;	/* 0x478 - Rx HDLC Buf Size */
	u32 reserved25[(0x494 - 0x478) / sizeof(u32)];
	u32 cpri_rethbdringsize; /* 0x494 - Rx Eth BD Ring Size */
	u32 cpri_tethbdringsize; /* 0x498 - Tx Eth BD Ring Size */
	u32 cpri_rhdlcbdringsize; /* 0x49C - Rx HDLC BD Ring Size */
	u32 cpri_thdlcbdringsize; /* 0x4A0 - Tx HDLC BD Ring Size */
	u32 reserved26[(0x4a8 - 0x4a0) / sizeof(u32)];
	u32 cpri_rgenmode;	/* 0x4A8 - Rx Gen CPRI Mode */
	u32 cpri_tgenmode;	/* 0x4AC - Tx Gen CPRI Mode */
	u32 reserved27[(0x4b8 - 0x4aC) / sizeof(u32)];
	u32 cpri_tbufsize;	/* 0x4B8 - Tx CPRI Framer Buf Size */
	u32 cpri_tctrlinserttb1; /* 0x4BC - Tx Ctrl Tbl Insert En 1 */
	u32 cpri_tctrlinserttb2; /* 0x4C0 - Tx Ctrl Tbl Insert En 2 */
	u32 reserved28[(0x4c8 - 0x4c0) / sizeof(u32)];
	u32 cpri_timer_cfg;	/* 0x4C8 - Timer Configuration */
	u32 cpri_rframepulsewidth; /* 0x4CC - Rx Frame Pulse Width */
	u32 cpri_tframepulsewidth; /* 0x4D0 - Tx Frame Pulse Width */
	u32 reserved29[(0x510 - 0x4d0) / sizeof(u32)];
	u32 cpri_rvssbaddr;	/* 0x510 - Rx VSS Base Address */
	u32 cpri_rvssbaddrmsb;	/* 0x514 - Rx VSS Base Address MSB */
	u32 reserved30[(0x520 - 0x514) / sizeof(u32)];
	u32 cpri_tvssbaddr;	/* 0x520 - Tx VSS Base Address */
	u32 cpri_tvssbaddrmsb;	/* 0x524 - Tx VSS Base Address MSB */
	u32 reserved31[(0x530 - 0x524) / sizeof(u32)];
	u32 cpri_rethbdringbaddr; /* 0x530 - Rx Ethernet BD Ring Base Add */
	u32 cpri_rethbdringbaddrmsb; /* 0x534 - Rx Eth BD Ring baddr MSB */
	u32 reserved32[(0x540 - 0x534) / sizeof(u32)];
	u32 cpri_tethbdringbaddr; /* 0x540 - Tx Eth BD Ring baddr */
	u32 cpri_tethbdringbaddrmsb; /* 0x544 - ,, MSB */
	u32 reserved33[(0x550 - 0x544) / sizeof(u32)];
	u32 cpri_rhdlcbdringbaddr; /* 0x550 - Rx HDLC BD Ring Base Addr */
	u32 cpri_rhdlcbdringbaddrmsb; /* 0x554 - ,, MSB */
	u32 reserved34[(0x560 - 0x554) / sizeof(u32)];
	u32 cpri_thdlcbdringbaddr; /* 0x560 - Tx HDLC BD Ring Base Addr */
	u32 cpri_thdlcbdringbaddrmsb; /* 0x564 - ,, MSB */
	u32 reserved35[(0x5f0 - 0x564) / sizeof(u32)];
	u32 cpri_auxctrl;	/* 0x5F0 - CPRI Aux Control */
	u32 reserved36[(0x600 - 0x5f0) / sizeof(u32)];
	/* Control registers */
	u32 cpri_rctrl;		/* 0x600 - Rx Control */
	u32 cpri_tctrl;		/* 0x604 - Tx Control */
	u32 cpri_raxcctrl;	/* 0x608 - Rx AxC Control */
	u32 reserved37[(0x610 - 0x608) / sizeof(u32)];
	u32 cpri_taxcctrl;	/* 0x610 - Tx AxC Control */
	u32 reserved38[(0x64c - 0x610) / sizeof(u32)];
	u32 cpri_rvssthresh;	/* 0x64C - Tx VSS Threshold */
	u32 cpri_tvssthresh;	/* 0x650 Tx Eth Coal Threshold */
	u32 cpri_rethcoalthresh; /* 0x654 - Rx Ethx Coalx Threshold */
	u32 cpri_tethcoalthresh; /* 0x658 - - Rx VSS Threshold */
	u32 reserved39[(0x660 - 0x658) / sizeof(u32)];
	u32 cpri_rctrltiminginten; /* 0x660 - CPRI Rx Ctrl & Timing Int En */
	u32 cpri_tctrltiminginten; /* 0x664 - CPRI Tx Ctrl & Timing Int En */
	u32 reserved40[(0x66c - 0x664) / sizeof(u32)];
	u32 cpri_eccerrindicateen; /* 0x66C - CPRI ECC Error Indication En */
	u32 cpri_errinten;	/* 0x670 - CPRI Error Interrupt En */
	u32 cpri_timeren;	/* 0x674 - Timer Enable */
	u32 cpri_rethwriteptr;	/* 0x678 - Rx Ethernet Write Ptr Ring */
	u32 cpri_tethwriteptr;	/* 0x67C - Tx Ethernet Write Ptr Ring */
	u32 cpri_rhdlcwriteptr;	/* 0x680 - Rx HDLC Write Ptr Ring */
	u32 cpri_thdlcwriteptr;	/* 0x684 - Tx HDLC Write Ptr Ring */
	u32 reserved41[(0x6a0 - 0x684) / sizeof(u32)];
	u32 cpri_rctrlattrib;	/* 0x6A0 - Rx Ctrl Attribute Register */
	u32 cpri_rctrldata0;	/* 0x6A4 - Rx Ctrl Data register 0 */
	u32 cpri_rctrldata1;	/* 0x6A8 - Rx Ctrl Data register 1*/
	u32 cpri_rctrldata2;	/* 0x6AC - Rx Ctrl Data Register 2 */
	u32 cpri_rctrldata3;	/* 0x6B0 - Rx Ctrl Data Register 3 */
	u32 cpri_tctrlattrib;	/* 0x6B4 - Tx Ctrl Attribute Register */
	u32 cpri_tctrldata0;	/* 0x6B8 - Tx Ctrl Data Register 0 */
	u32 cpri_tctrldata1;	/* 0x6BC - Tx Ctrl Data register 1 */
	u32 cpri_tctrldata2;	/* 0x6C0 - Tx Ctrl Data Register 2 */
	u32 cpri_tctrldata3;	/* 0x6C4 - Tx Ctrl Data Register 3 */
	u32 reserved42[(0x700 - 0x6c4) / sizeof(u32)];
	u32 cpri_raxcparam[24];	/* 0x700 to 0x75F - Rx Ant Carrier Param */
	u32 reserved43[(0x780 - 0x75f) / sizeof(u32)];
	u32 cpri_raxcparammsb[24];	/* 0x780 to 0x7DC  - ,, MSB */
	u32 reserved44[(0x900 - 0x7dc) / sizeof(u32)];
	u32 cpri_taxcparam[24];	/* 0x900 to 0x95F - Tx Ant Carrier Param */
	u32 reserved45[(0x980 - 0x95f) / sizeof(u32)];
	u32 cpri_taxcparammsb[24]; /* 0x980 to 0x9DF - ,, Param MSB */
	u32 reserved46[(0xb00 - 0x9df) / sizeof(u32)];
	u32 cpri_auxmask;	/* 0xB00 to 0xBFC - Aux Interface Mask */
	u32 reserved47[(0xc00 - 0xbfc) / sizeof(u32)];
	u32 cpri_tcfgmemaddr0;	/* 0xC00 - Tx Config Memory Address */
	u32 cpri_tcfgmemaddr1;	/* 0xC04 - Tx Config Memory Address */
	u32 cpri_rcfgmemaddr0;	/* 0xC08 - Rx Config Memory Data */
	u32 cpri_rcfgmemaddr1;	/* 0xC0C - Rx Config Memory Data */
	u32 cpri_raxierrstatus;	/* 0xC10 - Rx AXI Error Status */
	u32 cpri_taxierrstatus;	/* 0xC14 - Tx AXI Error Status */
	u32 cpri_raxierrinten;	/* 0xC18 - Rx AXI Error Int En */
	u32 cpri_taxierrinten;	/* 0xC1C - Tx AXI Error Int En */
	u32 cpri_raxciqthreshintstatus;	/* 0xC20 - Rx AxC IQ Thr Int Status */
	u32 cpri_taxciqthreshintstatus;	/* 0xC24 - Tx AxC IQ Thr Int Status */
	u32 cpri_raxciqthreshinten; /* 0xC28 - Rx AxC IQ Threshold Int En */
	u32 cpri_taxciqthreshinten; /* 0xC2C - Tx AxC IQ Threshold Int En */
	u32 cpri_tcfgmemaddr;	/* 0xC30 - Tx Config Memory Address */
	u32 cpri_rcfgmemaddr;	/* 0xC34 - Rx Config Memory Address */
	u32 cpri_ethfwdctrl;	/* 0xc38 - CPRI Eth Forward Inter Ctrl */
	u32 reserved48[(0xc40 - 0xc38) / sizeof(u32)];
	u32 cpri_auxcwdmask[8];	/* 0xC40-0xC5C - CPRI Aux Interface CWM */
	u32 cpri_auxcwdmasken;	/* 0xC60 - Aux Interface CWM En */
	u32 cpri_auxcwd130mask;	/* 0xC64 - Aux Interface CW 130 Mask*/
	u32 cpri_framediffctrl;	/* 0xC68 - Frame Diff Ctrl */
	u32 cpri_cwddelay;	/* 0xC6C - Control Word Delay */
	u32 cpri_framediffstatus; /* 0xC70 - Frame Diff Status */
	u32 cpri_framediffdriftstatus; /* 0xC74 - Frame Diff Drift Status */
	/* Status registers */
	u32 reserved49[(0xd10 - 0xc74) / sizeof(u32)];
	u32 cpri_rvssbufdispl;	/* 0xD10 - Rx VSS Bufx Displacement */
	u32 cpri_tvssbufdispl;	/* 0xD14 - Tx VSS Bufxr Displacement */
	u32 cpri_rethbufdesc[2]; /* 0xD18 and 0xD1F - Rx Ethx Buf Descriptor*/
	u32 reserved50[(0xd20 - 0xd1f) / sizeof(u32)];
	u32 cpri_tethbufdesc[2]; /* 0xD20 and 0xD27 - Tx Ethx Buf Descriptor */
	u32 cpri_rethrdptrring;	/* 0xD28 - Rx Ethernet Read Ptr Ring */
	u32 cpri_tethrdptrring;	/* 0xD2C - Tx Eth Read Ptr Ring */
	u32 cpri_rhdlcbufdesc[2]; /* 0xD30 &  0xD37 - Rx HDLC Buf Descriptor */
	u32 cpri_thdlcbufdesc[2]; /* 0xD38 & 0xD3C - Tx HDLC Buf Descriptor */
	u32 cpri_rhdlcrdptrring; /* 0xD40 - Rx HDLC Read Pointer Ring */
	u32 cpri_thdlcrdptrring; /* 0xD44 - Txt HDLC Read Pointer Ring */
	u32 cpri_revent;	/* 0xD48 - Rx Event Register */
	u32 cpri_tevent;	/* 0xD4C - Tx Event Register */
	u32 cpri_errevent;	/* 0xD50 - Errx Event Register */
	u32 cpri_rethcoalstatus; /* 0xD54 - Rx Ethernet Coalescing Status */
	u32 cpri_tethcoalstatus; /* 0xD58 - Tx Ethernet Coalescing Status */
	u32 cpri_timerstatus;	/* 0xD5C - TIMERn Status */
	u32 cpri_rstatus;	/* 0xD60 - Rx Status */
	u32 cpri_tstatus;	/* 0xD64 - Tx Status  */
	u32 cpri_multibiteccerrstatus; /* 0xD68 - Multi Bit ECC Error Status */
	u32 reserved51[(0xd70 - 0xd68) / sizeof(u32)];
	u32 cpri_thfnctr;	/* 0xD70 - CPRI Tx HFN Counter */
	u32 cpri_tbfnctr;	/* 0xD74 - CPRI Tx BFN Counter */
	u32 reserved52[(0xF00 - 0xd74) / sizeof(u32)];
	u32 cpri_raxciqvspthreshinten;	/* 0xF00 - Rx AxC IQ VSP Thr Int En */
	u32 cpri_taxciqvspthreshinten; /* 0xF04 - Tx AxC IQ VSP Thr Int En */
	u32 reserved53[(0xf20 - 0xf04) / sizeof(u32)];
	u32 cpri_raxcuruninten;	/* 0xF20 - Rx Ant Carr Underrun Int En */
	u32 cpri_raxcurunintstatus; /* 0xF24 - Rx Ant Carr Urun Int Status */
	u32 cpri_taxcoruninten;	/* 0xF28 - Tx Ant Carr Orun Int En */
	u32 cpri_taxcorunintstatus; /* 0xF2C - Tx Ant Carr Orun Int Status */
};

struct cpri_map_sync_offset {
	u8 rx_bfn_offset;
	u8 rx_hfn_offset;
	u8 tx_bfn_offset;
	u8 tx_hfn_offset;
	u8 start_tx_hfn_offset;
	u8 start_tx_bfn_offset;
};

struct cpri_framer_param {
	u32 framer_pram_flags;
	unsigned int max_axc_count;
	u32 rx_vss_axi_transac_size;
	u32 tx_vss_axi_transac_size;
	u32 tx_framer_buf_size;
	unsigned int k0;
	unsigned int k1;
	struct cpri_map_sync_offset map_sync_offset;
};

struct cpri_framer_stats {
	u8 tx_hfn;
	u16 tx_bfn;
	u8 rx_hfn;
	u16 rx_bfn;
	/* VSS Rx & Tx stats */
	u32 tx_vss_frames;
	u32 rx_vss_frames;
	/* Autoneg error stats */
	u32 linerate_autoneg_failures;
	u32 proto_autoneg_failures;
	u32 eth_autoneg_failures;
	u32 vendor_config_failures;
	/* Framer error stats */
	u32 rx_iq_overrun_err_count;
	u32 tx_iq_underrun_err_count;
	u32 rx_eth_mem_overrun_err_count;
	u32 tx_eth_underrun_err_count;
	u32 rx_eth_bd_underrun_err_count;
	u32 rx_hdlc_overrun_err_count;
	u32 tx_hdlc_underrun_err_count;
	u32 rx_hdlc_bd_underrun_err_count;
	u32 rx_vss_overrun_err_count;
	u32 tx_vss_underrun_err_count;
	u32 ecc_config_mem_err_count;
	u32 ecc_data_mem_err_count;
	u32 rx_eth_dma_overrun_err_count;
	u32 eth_fwd_rem_fifo_full_err_count;
	u32 ext_sync_loss_err_count;
	u32 rlos_err_count;
	u32 rlof_err_count;
	u32 rai_err_count;
	u32 rsdi_err_count;
	u32 llos_err_count;
	u32 llof_err_count;
	u32 rr_err_count;
	u32 fa_err_count;
	u32 rra_err_count;
	/* CPRIn_LCV */
	u32 rx_line_coding_violation;
};

struct framer_info {
	enum cpri_state state;
	u32 test_flags;
	/* CPRIn_BFN */
	u16 cur_bfn;
	/* CPRIn_HFN */
	u8 cur_hfn;
	struct cpri_framer_param framer_param;
	unsigned int max_axc_count;
	/* CPRIn_STATUS, CPRInRSR & CPRInTSR */
	u32 cpri_status_flags;
	/* CPRIn_HW_RESET */
	u8 hw_reset_status_flags;
	/* CPRInTXHFN */
	u8 tx_hfn_counter;
	/* CPRInTXBFN */
	u8 tx_bfn_counter;
};

struct cpri_framer {
	/* Overall data structures per framer */
	unsigned long id;
	struct cpri_framer_regs __iomem *regs;
	struct cpri_dev *cpri_dev;
	struct cdev cdev;
	dev_t dev_t;
	raw_spinlock_t regs_lock;
	struct device_node *sfp_dev_node;
	struct sfp_dev *sfp_dev;
	/* ISR events and bottom half */
	unsigned int irq_rx_t;
	unsigned int irq_tx_t;
	unsigned int irq_rx_c;
	unsigned int irq_tx_c;
	unsigned int timer_expiry_events;
	/* Configuration and stats */
	u32 test_flags;
	enum cpri_state framer_state;
	struct cpri_framer_stats stats;
	struct cpri_framer_param framer_param;
	struct cpri_framer *chained_framer;
	/* Autoneg data structures per framer*/
	struct cpri_autoneg_param autoneg_param;
	struct cpri_autoneg_output autoneg_output;
	/* Delay data structures */
	struct cpri_delay_config delay_cfg;
	struct cpri_delay_output delay_out;
	/* AxC data structures per framer */
	unsigned int max_axcs;
	u32 *axc_enable_bitmap;
	struct axc *axcs;
	struct axc_map_table ul_map_table;
	struct axc_map_table dl_map_table;
	raw_spinlock_t ul_map_tbl_lock;
	raw_spinlock_t dl_map_tbl_lock;
	struct axc_buf_head tx_buf_head;
	struct axc_buf_head rx_buf_head;
	/* Ethernet data structures per framer */
	struct cpri_eth_priv *eth_priv;
	/* VSS data structures per framer */
	struct vss_buf_ctrl  vss_tx_ctrl;
	struct vss_buf_ctrl  vss_rx_ctrl;
	u8 rx_vss_axi_transaction_size;
	u8 tx_vss_axi_transaction_size;
	/* Control word channel data structures per framer */
	struct hf_ctrl_chans tx_hf_ctrl_chans;
	struct hf_ctrl_chans rx_hf_ctrl_chans;
	raw_spinlock_t tx_cwt_lock;
	/* Daisy chain data structures per framer */
	struct daisy_chain_param chain_param;
	/* SRC */
	int src_bypass_status;
	unsigned int src_bypass_dur_sec;
	/* Poller variables */
	unsigned long link_poll_dur_sec;
	unsigned long passive_poll_dur_sec;
	unsigned long l1_expiry_dur_sec;
	struct timer_list link_poller;
	struct timer_list ethptr_poller;
	struct timer_list l1_timer;
	/* misc */
	unsigned char notification_state;
};
/* Per CPRI framer data structures - End */

/* Register R/W data structure */
struct regrw {
	u32 offset;
	unsigned int length;
	u32 buf[];
};

struct cpri_event_listener {
	u32 evt_mask;
	unsigned int signal;
};

struct cpri_reg_data {
	u32 mask;
	u32 val;
};

/* test_flags */
#define RX_TRANSPARENT_MODE			0x80000000
#define TX_TRANSPARENT_MODE			0x40000000

/* framer_pram_flags */
/* CPRIn_CONFIG */
#define SET_10_ACKS				0x80000000
#define CNT_6_RESET				0x40000000
#define SYNC_PULSE_MODE				0x20000000
#define TX_CW_INSERT				0x10000000
/* CPRInCWMASKEN */
#define CW					0x08000000
#define CW130					0x04000000
/* SRC */
#define RST_REQ_BYP				0x02000000
/* CPRIRROCR */
#define C1_REM_RES_OP				0x01000000
#define C1_REM_RES_ACK_OP			0x00800000
#define C2_REM_RES_OP				0x00400000
#define C2_REM_RES_ACK_OP			0x00200000
/* CPRInRCR */
#define RX_IQ_SYNC				0x00100000
/* CPRInTCR */
#define TX_IQ_SYNC				0x00080000
/* CPRInEFICR */
#define ETH_FWD					0x00040000
/* CPRIGRSR & CPRIGTSR */
#define GEN_RX_IQ_SYNC				0x00020000
#define GEN_TX_IQ_SYNC				0x00010000
/* CPRInECCEIER */
#define SINGLE_BIT_ECC_ERROR_OUTPUT		0x00008000
#define MULTI_BIT_ECC_ERROR_OUTPUT		0x00004000
/* Daisy chain */
#define AUX_MODE				0x00002000
/* Operation mode */
#define SLAVE_MODE				0x00001000
/* AxC mapping mode */
#define ADVANCED_MAP_MODE			0x00000800

/* cpri_status_flags */
/* CPRIn_STATUS */
#define RX_LOS_STATUS				0x80000000
#define RX_STATE_STATUS				0x40000000
#define RX_HFN_STATE_STATUS			0x20000000
#define RX_BFN_STATE_STATUS			0x10000000
#define RX_LOS_HOLD_STATUS			0x08000000
#define RX_STATE_HOLD_STATUS			0x04000000
#define RX_FREQ_ALARM_HOLD_STATUS		0x02000000
#define RX_RFP_HOLD_STATUS			0x01000000
/* CPRInRSR & CPRInTSR */
#define RX_VSS_EVENT_STATUS			0x00800000
#define TX_VSS_EVENT_STATUS			0x00400000
#define RX_ETH_EVENT_STATUS			0x00200000
#define TX_ETH_EVENT_STATUS			0x00100000
#define RX_IQ_EVENT_STATUS			0x00080000
#define TX_IQ_EVENT_STATUS			0x00040000
/* hw_reset_status_flags */
/* CPRIn_HW_RESET */
#define RESET_GEN_DONE_HOLD_STATUS		0x80000000
#define RESET_GEN_DONE_STATUS			0x40000000
#define RESET_DETECT_HOLD_STATUS		0x20000000
#define RESET_DETECT_STATUS			0x10000000

/* autoneg_param_flags */
/* CPRIn_RX_SCR_SEED */
#define RX_SCRAMBLER_EN				0x80000000

/* Timer Expiry Event flags */
#define TEVENT_LINERATE_SETUP_TEXPIRED		0x80000000
#define TEVENT_PROTVER_SETUP_TEXPIRED		0x40000000
#define TEVENT_ETHRATE_SETUP_TEXPIRED		0x20000000

/* Rx AxC Param Flags */
#define AXC_RX_IQ_FORMAT_2                      0x80000000
#define AXC_RX_INTERLEAVING_EN                  0x40000000
#define AXC_RX_CONVERSION_9E2_EN                0x20000000
#define AXC_RX_OVERSAMPLING_2X                  0x10000000

/* Tx AxC Param Flags */
#define AXC_TX_IQ_FORMAT_2                      0x80000000
#define AXC_TX_INTERLEAVING_EN                  0x40000000
#define AXC_TX_OVERSAMPLING_2X                  0x20000000
#define AXC_TX_ROUNDING_EN                      0x10000000

/* Misc flags */
#define MASK_ALL				0xFFFFFFFF
#define BITS_PER_U32				32
#define REG_SIZE				0x20
#define MAX_FRAMERS_PER_COMPLEX			2
#define FRAMER_1_ID				0x1
#define FRAMER_2_ID				0x2
#define CLK_ENABLE				0x01
#define BF_W_SIZE_16				16
#define BF_W_SIZE_32				32
#define BF_W_SIZE_40				40
#define BF_W_SIZE_64				64
#define BF_W_SIZE_80				80
#define BF_W_SIZE_128				128
#define BF_IQ_BITS_240				240
#define BF_IQ_BITS_480				480
#define BF_IQ_BITS_600				600
#define BF_IQ_BITS_960				960
#define BF_IQ_BITS_1200				1200
#define BF_IQ_BITS_1920				1920

/* ------------- Register masks - Start ------------- */
/* CPRI Status Register (CPRIn_STATUS) */
#define RX_LOS_MASK				0x1
#define RX_STATE_MASK				0x2
#define RX_HFN_STATE_MASK			0x4
#define RX_BFN_STATE_MASK			0x8
#define RX_LOS_HOLD_MASK			0x100
#define RX_STATE_HOLD_MASK			0x200
#define RX_FREQ_ALARM_HOLD_MASK			0x400
#define RX_RFP_HOLD_MASK			0x800

/* CPRI Configuration (CPRIn_CONFIG) */
#define CONF_SET_10_ACKS_MASK			0x4000
#define CONF_CNT_6_RESET_MASK			0x800
#define CONF_SYNC_PULSE_MODE_MASK		0x400
#define CONF_RX_EN_MASK                         0x200
#define CONF_TX_EN_MASK				0x20
#define SLAVE_MODE_MASK				0x2
#define TX_CW_INSERT_EN_MASK			0x1

/* CPRI Receive Line Coding Violation Counter (CPRIn_LCV) */
#define CNT_LCV_MASK				0xFF

/* CPRI Recovered BFN Counter (CPRIn_BFN) */
#define RECOVERED_BFN_CNT_MASK			0xFFF

/* CPRI Recovered HFN Counter (CPRIn_HFN) */
#define RECOVERED_HFN_CNT_MASK			0xFF

/* CPRI Hardware Reset from Control Word (CPRIn_HW_RESET) */
#define RESET_GEN_DONE_HOLD_MASK		0x80
#define RESET_GEN_DONE_MASK			0x40
#define RESET_DETECT_HOLD_MASK			0x20
#define RESET_DETECT_MASK			0x10
#define RESET_OUT_EN_MASK			0x4
#define RESET_GEN_FORCE_EN_MASK			0x2
#define RESET_GEN_EN_MASK			0x1

/* CPRI Control and Management Configuration (CPRIn_CM_CONFIG) */
#define TX_FAST_CM_PTR_MASK			0x3F

/* CPRI Control and Management Status (CPRIn_CM_STATUS) */
#define RX_FAST_CM_PTR_VALID_MASK		0x40
#define RX_FAST_CM_PTR_MASK			0x3F

/* CPRI Receive Delay (CPRIn_RX_DELAY) */
#define RX_ALIGN_DELAY_MASK			0x7F0000
#define RX_BUF_DELAY_MASK			0x3FC
#define RX_BYTE_DELAY_MASK			0x3

/* CPRI Round Trip Delay (CPRIn_ROUND_DELAY) */
#define RX_ROUND_TRIP_DELAY_MASK		0xFFFFF

/* CPRIn_EX_DELAY_CONFIG */
#define TX_EX_DELAY_MASK			0x3F0000
#define RX_EX_DELAY_PERIOD_MASK			0x1FF

/* CPRIn_EX_DELAY_STATUS */
#define RX_EX_BUF_DELAY_VALID_MASK		0x80000000
#define RX_EX_BUF_DELAY_MASK			0x7FFF

/* CPRI Transmit Protocol Version (CPRIn_TX_PROT_VER) */
#define PROTO_VER_MASK				0xFF

/* CPRIn_TX_SCR_SEED & CPRIn_RX_SCR_SEED */
#define SCR_SEED_MASK				0x7FFFFFFF
#define RX_SCR_EN_MASK				0x80000000

/* CPRI Transmit Control (CPRIn_TX_CONTROL) */
#define TX_RESET_BFN_MASK			0x1

/* CPRI Mapping Configuration (CPRIn_MAP_CONFIG) */
#define TX_TRANSPARENT_MODE_MASK		0x80
#define RX_TRANSPARENT_MODE_MASK		0x40
#define MAP_MODE_MASK				0x3

/* CPRIn_MAP_OFFSET_TX, CPRIn_MAP_OFFSET_RX & CPRIn_START_OFFSET_TX */
#define MAP_OFFSET_X_MASK			0xFF
#define MAP_OFFSET_Z_MASK			0xFF00

/* CPRInRVSSMTS & CPRInTVSSMTS */
#define AXI_TRANSAC_SIZE_MASK			0xF

/* Transmit CPRI Framer Buffer Size(CPRInTCFBS) */
#define FR_BUF_SIZE_MASK			0x7F

/* CPRI Auxiliary Control Register (CPRInAUXCR) */
#define AUX_MODE_MASK				0x1

/* Rx & Tx Control Register (CPRInRCR/CPRInTCR)) */
#define IQ_EN_MASK				0x1
#define ETH_EN_MASK				0x2
#define HDLC_EN_MASK				0x4
#define VSS_EN_MASK				0x8
#define IQ_SYNC_EN_MASK				0x8000

/* CPRInRCIER & CPRInTCIER */
#define TIMING_INT_LEVEL_MASK			0x80000000
#define BFN_TIMING_EVENT_EN_MASK		0x20000
#define HFN_TIMING_EVENT_EN_MASK		0x10000
#define CONTROL_INT_LEVEL_MASK			0x8000
#define ETH_EVENT_EN_MASK			0x4
#define HDLC_EVENT_EN_MASK			0x2
#define VSS_EVENT_EN_MASK			0x1

/* CPRI ECC Error Indication Enable Register (CPRInECCEIER) */
#define SINGLE_BIT_ECC_ERROR_OUTPUT_EN_MASK	0x1
#define MULTI_BIT_ECC_ERROR_OUTPUT_EN_MASK	0x2

/* CPRInCWMASKEN */
#define CW_EN_MASK				0x1
#define CW130_EN_MASK				0x2

/* CPRInEIER & CPRInEER */
#define IEVENT_RX_IQ_OVERRUN			0x1
#define IEVENT_TX_IQ_UNDERRUN			0x2
#define IEVENT_RX_ETH_MEM_OVERRUN		0x4
#define IEVENT_TX_ETH_UNDERRUN			0x8
#define IEVENT_RX_ETH_BD_UNDERRUN		0x10
#define IEVENT_RX_HDLC_OVERRUN			0x20
#define IEVENT_TX_HDLC_UNDERRUN			0x40
#define IEVENT_RX_HDLC_BD_UNDERRUN		0x80
#define IEVENT_RX_VSS_OVERRUN			0x100
#define IEVENT_TX_VSS_UNDERRUN			0x200
#define IEVENT_ECC_CONFIG_MEM			0x400
#define IEVENT_ECC_DATA_MEM			0x800
#define IEVENT_RX_ETH_DMA_OVERRUN		0x1000
#define IEVENT_ETH_FORWARD_REM_FIFO_FULL	0x2000
#define IEVENT_EXT_SYNC_LOSS			0x8000
#define IEVENT_RLOS				0x10000
#define IEVENT_RLOF				0x20000
#define IEVENT_RAI				0x40000
#define IEVENT_RSDI				0x80000
#define IEVENT_LLOS				0x100000
#define IEVENT_LLOF				0x200000
#define IEVENT_RRE				0x400000
#define IEVENT_FAE				0x800000
#define IEVENT_RRA				0x1000000

#define IEVENT_SFP_TXFAULT			0x2000000
#define IEVENT_SFP_LOS				0x4000000
#define IEVENT_SFP_PRS				0x8000000

#define CPRI_ERR_EVT_ALL	(IEVENT_RX_IQ_OVERRUN \
		| IEVENT_TX_IQ_UNDERRUN \
		| IEVENT_RX_ETH_MEM_OVERRUN \
		| IEVENT_TX_ETH_UNDERRUN \
		| IEVENT_RX_ETH_BD_UNDERRUN \
		| IEVENT_RX_HDLC_OVERRUN \
		| IEVENT_TX_HDLC_UNDERRUN \
		| IEVENT_RX_HDLC_BD_UNDERRUN \
		| IEVENT_RX_VSS_OVERRUN \
		| IEVENT_TX_VSS_UNDERRUN \
		| IEVENT_ECC_CONFIG_MEM \
		| IEVENT_ECC_DATA_MEM \
		| IEVENT_RX_ETH_DMA_OVERRUN \
		| IEVENT_ETH_FORWARD_REM_FIFO_FULL \
		| IEVENT_EXT_SYNC_LOSS \
		| IEVENT_RLOS \
		| IEVENT_RLOF \
		| IEVENT_RAI \
		| IEVENT_RSDI \
		| IEVENT_LLOS \
		| IEVENT_LLOF \
		| IEVENT_RRE \
		| IEVENT_FAE \
		| IEVENT_RRA)

/* CPRInRER & CPRInTER */
#define IEVENT_BFN_MASK				0x80
#define IEVENT_HFN_MASK				0x40
#define IEVENT_ETH_MASK				0x20
#define IEVENT_HDLC_MASK			0x10
#define IEVENT_VSS_THRESHOLD_MASK		0x8
#define IEVENT_IQ_THRESHOLD_MASK		0x1

/* CPRInRSR & CPRInTSR */
#define VSS_EVENT_EN_STATUS_MASK		0x8
#define HDLC_EVENT_EN_STATUS_MASK		0x4
#define ETH_EVENT_EN_STATUS_MASK		0x2
#define IQ_EVENT_EN_STATUS_MASK			0x1

/* Multi Bit ECC Error Status Register (CPRInMBEESR) */
#define ECC_ERR_ALL_MASK			0x3FFFFC
#define ECC_ERR_CW_DEL_MEM_MASK			0x200000
#define ECC_ERR_ETH_FWD_MEM_MASK		0x100000
#define ECC_ERR_TX_IQ_MEM1_MASK			0x80000
#define ECC_ERR_TX_IQ_MEM0_MASK			0x40000
#define ECC_ERR_RX_IQ_MEM1_MASK			0x20000
#define ECC_ERR_RX_IQ_MEM0_MASK			0x10000
#define ECC_ERR_RX_CFG_MEM_MASK			0x8000
#define ECC_ERR_TX_CFG_MEM_MASK			0x4000
#define ECC_ERR_FR_RX_IQ_MEM_MASK		0x2000
#define ECC_ERR_FR_TX_IQ_MEM_MASK		0x1000
#define ECC_ERR_RX_DMA_MEM_MASK			0x800
#define ECC_ERR_TX_DMA_MEM_MASK			0x400
#define ECC_ERR_RX_CWD_MEM_MASK			0x200
#define ECC_ERR_TX_CWD_MEM_MASK			0x100
#define ECC_ERR_FR_RX_ETH_MEM_MASK		0x80
#define ECC_ERR_FR_TX_ETH_MEM_MASK		0x40
#define ECC_ERR_FR_RX_HLC_MEM_MASK		0x20
#define ECC_ERR_FR_TX_HDLC_MEM_MASK		0x10
#define ECC_ERR_AUX_MEM_MASK			0x8
#define ECC_ERR_FR_ELASTIC_MEM_MASK		0x4

/* CPRI Transmit HFN Counter (CPRInTXHFN) */
#define TX_HFN_COUNTER_MASK			0xFF

/* CPRI Transmit BFN Counter (CPRInTXBFN) */
#define TX_BFN_COUNTER_MASK			0xFF

/* CPRI Control Clocks Registers (CPRICCR) */
#define C1_CLK_MASK				0x1
#define C2_CLK_MASK				0x2

/* CPRI Interrupt Control Register y (CPRIICR<y>) */
#define EVENT_REG_SELECT_MASK			0xF0000000
#define LEVEL_MASK				0x10000
#define IEVENT_BFN_FR_TIMING_EN_MASK		0x80
#define IEVENT_HFN_FR_TIMING_EN_MASK		0x40
#define IEVENT_ETH_EN_MASK			0x20
#define IEVENT_HDLC_MASK			0x10
#define IEVENT_VSS_MASK				0x8
#define IEVENT_IQ_THRESHOLD_MASK		0x1

/* General Receive Synchronization Register (CPRIGRSR) */
#define GEN_IQ_SYNC_MASK			0x1

/* CPRI Error Status Register (CPRIESR) */
#define C1_ERR_MASK				0x1
#define C2_ERR_MASK				0x2

/* CPRI Remote Reset Output Control Register (CPRIRROCR) */
#define C1_REM_RES_OP_EN_MASK			0x1
#define C2_REM_RES_OP_EN_MASK			0x2
#define C1_REM_RES_ACK_OP_EN_MASK		0x100
#define C2_REM_RES_ACK_OP_EN_MASK		0x200

/* Transmit Control Attribute Register (CPRInTCA)*/
#define TCT_ADDR_MASK				0x3FC00
#define TCT_WRITE_MASK				0x40000

/* ----------- Register masks - End ---------------- */

/* Control word constants */
#define CHAN_MAX		64
#define WDOFF_B0		0
#define MAX_CWBYTES		16
#define NUM_IDX			4
#define WORD_IDX0		0
#define WORD_IDX1		1
#define WORD_IDX2		2
#define WORD_IDX3		3
#define CWIDX_ETHPTR		3
#define CWIDX_PROTVER		0
#define FIND_CW_NUM(chan_no, cw_idx)	(chan_no + CHAN_MAX * cw_idx)
#define SIZE_BYTE		8
#define BYTE_MASK		0xFF
#define SIZE_REGBYTES		4

/* Autoneg IOCTLS */
#define CPRI_MAGIC 'C'
#define CPRI_START_AUTONEG			_IO(CPRI_MAGIC, 1)
#define CPRI_START_L1TIMER			_IO(CPRI_MAGIC, 2)
#define  CPRI_STOP_L1TIMER			_IO(CPRI_MAGIC, 3)
#define CPRI_CFG_L1TIMER			_IOW(CPRI_MAGIC, 4, long)
#define CPRI_SET_AUTONEG_PARAM			_IOW(CPRI_MAGIC, 5, \
						struct cpri_autoneg_param*)
#define CPRI_GET_AUTONEG_PARAM			_IOR(CPRI_MAGIC, 6, \
						struct cpri_autoneg_param*)
#define CPRI_GET_AUTONEG_OUTPUT			_IOR(CPRI_MAGIC, 7, \
						struct cpri_autoneg_output*)

/* Main driver IOCTLS */
#define CPRI_SET_STATE                          _IOW(CPRI_MAGIC, 20, \
						enum cpri_state)
#define CPRI_GET_STATE                          _IOR(CPRI_MAGIC, 21, \
						enum cpri_state)
#define CPRI_SET_TEST_MODE                      _IOW(CPRI_MAGIC, 22, \
						unsigned int)
#define CPRI_GET_TEST_MODE                      _IOR(CPRI_MAGIC, 23, \
						unsigned int)
#define CPRI_INIT_FRAMER			_IOW(CPRI_MAGIC, 24, \
						struct cpri_framer_param*)
#define CPRI_GET_FRAMER_INFO                    _IOR(CPRI_MAGIC, 25, \
						struct framer_info)
#define CPRI_GET_STATS                          _IOR(CPRI_MAGIC, 26, \
						struct cpri_framer_stats*)
#define CPRI_BFN_RESET                          _IO(CPRI_MAGIC, 27)
#define CPRI_REG_WRITE                          _IOW(CPRI_MAGIC, 28, \
						struct reg_rw*)
#define CPRI_REG_READ                           _IOR(CPRI_MAGIC, 29, \
						struct reg_rw*)
#define CPRI_REGISTER_EVENT			_IOW(CPRI_MAGIC, 30, \
						struct event_listener*)
#define CPRI_UNREGISTER_EVENT			_IO(CPRI_MAGIC, 31)
#define CPRI_EVENT_NOTIFICATION			_IOW(CPRI_MAGIC, 31, \
						unsigned char*)
#define CPRI_CFG_DELAY_RAW			_IOW(CPRI_MAGIC, 32, \
						struct cpri_delay_config*)
#define CPRI_GET_DELAY_RAW			_IOR(CPRI_MAGIC, 33, \
						struct cpri_delay_output*)

/* TODO: AxC IOCTLS */
/* TODO: VSS IOCTLS */
/* TODO: CW IOCTLS */

static inline u32 cpri_read(const void __iomem *addr)
{
	u32 val;
	val = readl(addr);
	return val;
}

static inline void cpri_write(u32 val, void __iomem *addr)
{
	writel(val, addr);
}

static inline void cpri_reg_clear(raw_spinlock_t *lock,
				void __iomem *addr,
				u32 mask)
{
	u32 val;
	unsigned long flags;

	raw_spin_lock_irqsave(lock, flags);
	val = cpri_read(addr);
	val &= ~mask;
	cpri_write(val, addr);
	raw_spin_unlock_irqrestore(lock, flags);
}

static inline void cpri_reg_set(raw_spinlock_t *lock,
				void __iomem *addr,
				u32 mask)
{
	u32 val;
	unsigned long flags;

	raw_spin_lock_irqsave(lock, flags);
	val = cpri_read(addr);
	val |= mask;
	cpri_write(val, addr);
	raw_spin_unlock_irqrestore(lock, flags);
}

static inline void cpri_reg_set_val(raw_spinlock_t *lock,
				void __iomem *addr,
				u32 mask,
				u32 val)
{
	u32 tmp = 0;
	unsigned long flags;
	register u32 cnt = 0;

	cnt = ffs(mask);

	raw_spin_lock_irqsave(lock, flags);
	tmp = cpri_read(addr);
	tmp |= mask;
	tmp &= (~mask  | (val << cnt));
	cpri_write(tmp, addr);
	raw_spin_unlock_irqrestore(lock, flags);
}

static inline u32 cpri_reg_get_val(raw_spinlock_t *lock,
				const void __iomem *addr,
				u32 mask)
{
	u32 val;
	unsigned long flags;
	register u32 cnt = 0;

	cnt = ffs(mask);

	raw_spin_lock_irqsave(lock, flags);
	val = cpri_read(addr);
	raw_spin_unlock_irqrestore(lock, flags);

	return (val & mask)>>cnt;
}

static inline void cpri_reg_vset_val(raw_spinlock_t *lock,
				void __iomem *addr,
				struct cpri_reg_data *data)
{
	u32 tmp = 0;
	u32 mask, val;
	unsigned long flags;
	register u32 cnt = 0;

	raw_spin_lock_irqsave(lock, flags);
	tmp = cpri_read(addr);

	while (data->mask != 0) {

		mask = data->mask;
		val = data->val;
		cnt = 0;

		cnt = ffs(mask);

		tmp |= mask;
		tmp &= (~mask  | (val << cnt));

		data++;
	}

	cpri_write(tmp, addr);
	raw_spin_unlock_irqrestore(lock, flags);
}

/* CPRI base prototypes */
long cpri_ioctl(struct file *fp, unsigned int cmd,
			unsigned long arg);
void eth_ptr_check_poller_hndlr(unsigned long ptr);
void link_check_poller_hndlr(unsigned long ptr);
void l1_timer_expiry_hndlr(unsigned long ptr);

/* Autoneg prototypes */
int cpri_autoneg_ioctl(struct cpri_framer *framer, unsigned int cmd,
			unsigned long arg);

/* Control word function prototypes */
int set_txethrate(u8 eth_rate, struct cpri_framer *framer);
int get_txethrate(struct cpri_framer *framer, u8 *eth_rate);
int set_txprotver(enum cpri_prot_ver ver, struct cpri_framer *framer);
int get_txprotver(struct cpri_framer *framer,
			enum cpri_prot_ver *prot_ver);
int get_rxethrate(struct cpri_framer *framer, u8 *eth_rate);
int get_rxprotver(struct cpri_framer *framer, enum cpri_prot_ver *prot_ver);

/* Ethernet exported functions */
extern int cpri_eth_init(struct platform_device *ofdev,
			struct cpri_framer *framer,
			struct device_node *frnode);
extern void cpri_eth_deinit(struct platform_device *ofdev,
		struct cpri_framer *framer);
extern void cpri_eth_enable(struct cpri_framer *framer);
extern int cpri_eth_handle_rx(struct cpri_framer *framer);
extern int cpri_eth_handle_tx(struct cpri_framer *framer);
extern int cpri_eth_handle_error(struct cpri_framer *framer);

/* SFP exported functions */
extern struct cpri_framer
	*get_attached_cpri_dev(struct device_node *sfp_dev_node);
extern struct sfp_dev *get_attached_sfp_dev(struct device_node *sfp_dev_node);
extern void set_sfp_txdisable(struct sfp_dev *sfp, unsigned value);

#endif /* __CPRIDRV_H */
