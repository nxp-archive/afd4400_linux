...
static void vspa_configure_irq_events(struct vspa_device *vspadev)
{
	...
	/* Disable all IRQ */
	iowrite32(0, regs + IRQEN_REG_OFFSET);
}
...
static ssize_t
vspa_read(struct file *fp, char __user *buf, size_t nbytes, loff_t *ppos)
{
	struct vspa_device *device = (struct vspa_device *)fp->private_data;
	...
	wait_event_interruptible(device->irq_wait_q, (device->status_reg > 0));
...
static irqreturn_t vspa_irq_handler(int irq, void *dev)
{
	...
	vspadev->status_reg =
		ioread32(vspadev->regs + STATUS_REG_OFFSET);
	if (vspadev->status_reg > 0) {
		wake_up_interruptible(&vspadev->irq_wait_q);
		/* disable irqs */
		iowrite32(0 , vspadev->regs + IRQEN_REG_OFFSET);
	}
...
static int vspa_register_irqs(struct vspa_device *vspadev)
{
	...
	rc = request_irq(vspadev->irq_vspa_done, vspa_irq_handler,
		flags, VSPA_DEVICE_NAME, vspadev);
	if (rc < 0)
		goto out;
...
static long vspa_ioctl(struct file *filp, unsigned int cmd, unsigned long data)
{
	...
	case IOCTL_IRQ_EN:
		enable_irq(dev->irq_dma_cmp);
		enable_irq(dev->irq_dma_err);
		enable_irq(dev->irq_vspa_done);
		enable_irq(dev->irq_vspa_msg);
			break;

	case IOCTL_IRQ_DIS:
		disable_irq(dev->irq_dma_cmp);
		disable_irq(dev->irq_dma_err);
		disable_irq(dev->irq_vspa_done);
		disable_irq(dev->irq_vspa_msg);
			break;
	}
	return rc;
}
...
static unsigned int vspa_poll(struct file *filp, poll_table *wait)
{
	...
	if (ioread32(dev->regs + STATUS_REG_OFFSET) > 0)
...
static int vspa_probe(struct platform_device *pdev)
{
	...
	if (vspa_major) {
		...
		pr_err("Device %s allocation failed %d\n",
					vspa_major ? "minor" : "major", result);
		result = -ENODEV;
		goto reg_fail;
	}
