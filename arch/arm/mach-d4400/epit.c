...
#define EPITCNR_START_VAL               0xffffffff
...
{
	if (unlikely(!sched_clock_reg))
		return 0;
	/* EPIT is a down counter, sched_clock should return an increasing
	 * value */
	return (u32) (EPITCNR_START_VAL - __raw_readl(sched_clock_reg));
}
...
static int __init epit_clocksource_init(struct clk *timer_clk)
{
	...
	setup_sched_clock(epit_read_sched_clock, 32, c);
		...
		struct clock_event_device *unused)
{
	...
	tcmp = __raw_readl(timer_base + EPITCNR) - evt;

	__raw_writel(tcmp, timer_base + EPITCMPR);

	return (int) (__raw_readl(timer_base + EPITCNR) - tcmp) < 0 ?
		-ETIME : 0;
}
		...
		struct clock_event_device *evt)
{
	...
	if (mode != clockevent_mode) {
		...
		__raw_writel(__raw_readl(timer_base + EPITCNR) + 3,
				timer_base + EPITCMPR);
	...
	case CLOCK_EVT_MODE_PERIODIC:
		pr_err("epit_set_mode: Periodic mode is not supported for D4400 EPIT\n");
		break;
